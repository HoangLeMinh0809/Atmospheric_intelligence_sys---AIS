...
CURRENT_DATE="${CURRENT_DATE:-$END_DATE}"
HISTORICAL_END_DATE="${HISTORICAL_END_DATE:-$(days_before_utc "$CURRENT_DATE" 1)}"
START_DATE="${START_DATE:-$(days_before_utc "$HISTORICAL_END_DATE" "$LOOKBACK_DAYS")}"

if [[ "$START_DATE" > "$HISTORICAL_END_DATE" ]]; then
  echo "[ERROR] START_DATE=${START_DATE} must be on or before HISTORICAL_END_DATE=${HISTORICAL_END_DATE} (CURRENT_DATE - 1 day)." >&2
  exit 1
fi

WINDOW_START_UTC="${WINDOW_START_UTC:-${START_DATE}T00:00:00Z}"
WINDOW_END_UTC="${WINDOW_END_UTC:-${HISTORICAL_END_DATE}T23:59:59Z}"
...
ERA5_END_DATE="${ERA5_END_DATE:-$HISTORICAL_END_DATE}"
...
echo "Requested current/live date: ${CURRENT_DATE}"
echo "Historical date window: ${START_DATE} -> ${HISTORICAL_END_DATE}"
